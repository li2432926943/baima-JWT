...
public class JwtUtils {
    ...
    public String createJwt(UserDetails user, String username, int userId) {
        if(this.frequencyCheck(userId)) {
            ...
            // 将authorities转换为字符串数组，不要去除ROLE_前缀
     ...
     * @return DecodedJWT
     */
    //这里返回 DecodedJWT，就是返回一个已解析的 JWT 令牌对象
    //resolveJwt解析Jwt令牌
    ...
    public UserDetails toUser(DecodedJWT jwt) {
        ...
        // 打印调试信息
        System.out.println("JWT中提取的用户名: " + username);
        System.out.println("JWT中提取的权限: " + Arrays.toString(authorities));
     ...
     * @return 转换后的令牌
     */
    //转换token。比如：输入："Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..."。输出："eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..
